    ...
    public function ResponsableDashboard()
{
    ...
    return view('responsable.index', compact('totalProduction', 'totalSinistreDim', 'totalSinistreAt_Rd', 'sinistres_dim', 'sinistres', 'productions','users'));
}
...
if($request->file('photo')){
        ...
        @unlink(public_path('upload/responsable_images'.$data->photo));
        $filename = date('YmdHi').$file->getClientOriginalName();
        $file->move(public_path('upload/responsable_images'), $filename);
        $data['photo'] = $filename;
    }
    
$data->save();
$notification = array(
    'message' => 'Responsable Profile Updated Successfully',
    'alert-type' => 'success'
);
...
$id = Auth::user()->id;
$profileData = User::find($id);
return view('responsable.responsable_change_password', compact('profileData'));
}
